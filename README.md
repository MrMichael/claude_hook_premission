...
| 功能 | 说明 |
|------|------|
| **自动放行** | 按工具+命令自动批准权限请求，跳过 Claude Code 交互式确认 |
| **弹窗确认** | 危险命令弹出 GUI 窗口（zenity），用户一键确认或拒绝 |
| **硬拒绝** | 系统级破坏性命令直接阻止，不弹窗 |
| **审计日志** | JSON 行式日志，每条决策记录时间戳、工具、命令、判断、原因 |
| **桌面通知** | notify-send 实时通知决策结果，三种模式可调（静默/详细/关闭） |
| **空闲提醒** | Claude 响应完毕后自动弹窗"任务完成，等待输入"，10 秒自动消失 |
| **自动拉起** | 守护进程未运行时客户端自动启动，消除死锁 |
| **单一实例** | PID 文件保障，重复启动自动跳过 |
